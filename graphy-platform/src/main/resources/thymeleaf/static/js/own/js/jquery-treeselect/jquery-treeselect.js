(function ($) {
    ...
    var init_tree_container = function () {
        ...
        this.tree_container =
            $('<div   class="menuContent" style="display:none;background-color: white; overflow-y: auto; position: absolute;z-index:110004;border: solid 1px;border-radius: 5px;padding-bottom: 10px; max-height:' + opt.maxHeight + 'px; " >'
                + '<input class="searchInput" placeholder="输入关键字查询" type="text" style="margin-top: 4px;border-radius:3px;width: ' + getelementwidth(this.$target_element) + '"><ul id="' + this.ztreeid + '" class="ztree" style="padding:0 0;margin-top:0;border: none;border-radius: 5px;  height:' + this.options.height + '; width:' + getelementwidth(this.$target_element) + '; background-color: white"></ul>' +
                '</div>').insertAfter(this.$target_element);
        this.checked_val_element = $('<input type="hidden" name="' + this.$target_element.nameLabel + '">').insertAfter(this.tree_container);
